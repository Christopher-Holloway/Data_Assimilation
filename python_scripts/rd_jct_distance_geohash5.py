...
class DistanceCalculator(object):
	def __init__(self):
		self.geo_index = GeoGridIndex(precision=4)
	...
	def load_index(self, input=None):
		...
		for line in open(input,'rU'):
			...
			if admin1 == 'VA' and tag == 'RDJCT' and 'Interchange' in title:
				#print lat
				self.geo_index.add_point(GeoPoint(lat,lon))
				input_counter +=1
		print 'Done loading index of office building (added %s values)' %(input_counter)
	...
	def enumerate_all_distances(self, admin1=None):
		...
		for geo5, lat, lon in geohashes_plus_coords:
			...
			values = self.geo_index.get_nearest_points(temp_geo_point, 20.0, 'km')
			#print values
			...
			#and write the distance value to the database.
			c.execute('UPDATE boxes set road_jct_distance=? where geohash=?',(minimum_distance,geo5))
		self.conn.commit()
