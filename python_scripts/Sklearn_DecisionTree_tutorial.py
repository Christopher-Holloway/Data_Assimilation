...
default = cross_val_score(clf, X, Y, cv=10, n_jobs=4)
print("mean: {:.3f} (std: {:.3f})".format(default.mean(),
                                          default.std()),
                                          end="\n\n" )

y_pred = cross_val_predict(clf, X, Y, cv=10, n_jobs=4)
...
param_grid = {'criterion': ['gini','entropy'],
        'class_weight':['balanced'],
				...
				'random_state': [7],
				'max_depth': [3,4,5,10,50],
				'min_samples_leaf': [5,10,20],
				'max_leaf_nodes': [None, 5,10,15]}

dt = DecisionTreeClassifier()
ts_gs = run_gridsearch(X, Y, dt, param_grid, cv=10, n_jobs=4)
...
y_pred_gs = cross_val_predict(dt_ts_gs, X, Y, cv=10, n_jobs=4)
...
y_pred_rs = cross_val_predict(dt_ts_rs, X, Y, cv=10, n_jobs=4)
