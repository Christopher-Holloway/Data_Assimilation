...
def run_gridsearch(X, y, clf, param_grid, cv=5, n_jobs=4):
...
clf = AdaBoostClassifier(base_estimator=DecisionTreeClassifier(class_weight='balanced', random_state=7)) 
default = cross_val_score(clf, X_sparse, Y, cv=10, n_jobs=4)
print("mean: {:.3f} (std: {:.3f})".format(default.mean(),
                                          default.std()),
                                          end="\n\n" )

y_pred = cross_val_predict(clf, X_sparse, Y, cv=10, n_jobs=4)
cm_default = contingency_matrix(y_pred)

param_grid = {'base_estimator': [DecisionTreeClassifier(class_weight={0:0.8, 1:.2})],
...
ts_gs = run_gridsearch(X_sparse, Y, dt, param_grid, cv=10, n_jobs=4)
...
y_pred_gs = cross_val_predict(dt_ts_gs, X_sparse, Y, cv=10, n_jobs=4)
...
dt_ts_rs = AdaBoostClassifier(**ts_rs)
scores = cross_val_score(dt_ts_rs, X_sparse, Y, cv=10)
print("mean: {:.3f} (std: {:.3f})".format(scores.mean(),
                                          scores.std()),
                                          end="\n\n" )

y_pred_rs = cross_val_predict(dt_ts_rs, X_sparse, Y, cv=10, n_jobs=4)
