...
def run_gridsearch(X, y, clf, param_grid, cv=5, njobs=4):
...
default = cross_val_score(clf, X, Y, cv=10)
...
ts_gs = run_gridsearch(X, Y, dt, param_grid, cv=10, njobs=4)
...
dt_ts_gs = KNeighborsClassifier(**ts_gs)
scores = cross_val_score(dt_ts_gs, X, Y, cv=10)
print("mean: {:.3f} (std: {:.3f})".format(scores.mean(),
                                          scores.std()),
                                          end="\n\n" )

y_pred_gs = cross_val_predict(dt_ts_gs, X, Y, cv=10)
...
dt_ts_rs = KNeighborsClassifier(**ts_rs)
